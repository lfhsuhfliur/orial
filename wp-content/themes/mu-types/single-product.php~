...
$reference = get_meta_option('product_reference');
$ontop = get_meta_option('product_ontop');
$currency = get_theme_option('product','currency');
$buy_now = get_theme_option('product','buy_now');
$desc = get_the_excerpt();
$quantities = explode("/", get_meta_option('product_quantities'));
...
<div class="post post-single post-product-single" id="post-<?php the_ID(); ?>">
	...
	<h2><?php the_title(); ?> &nbsp;&nbsp;&nbsp; <?php if($reference) : ?>ref. <?php echo $reference; ?><?php endif; ?></h2>
	<p>ontop: <?php echo $ontop ?></p>
	...
	<?php if($quantities[0]) : ?>
		<span>Quantità: sottoimballo <?php echo $quantities[0]; ?>
		<?php if($quantities[1]) : ?>
		, imballo<?php echo $quantities[1]; ?>
		<?php endif; ?>
		.</span>
	<?php endif; ?>
