...
class pageflipFunctions
{
   var $main;

   function pageflipFunctions()
   {
   }
	...
	function buildImage( $images, $page, $width, $height, $layoutId, $quality=75 )
	{
		  ...
		  foreach( $this->main->layouts[(int)$layoutId]->areas as $area )
		  {
				...
				$image_path = str_replace($this->main->siteURL.'/', ABSPATH, $images[(int)$area->id]);
	...
	function getThumb( $image )
	{
		...
		if ( preg_match('/(q_\d+_\d+x\d+_[0-9a-f]+_)(.*)/i', $pathParts['basename'], $m) )
			$basename = $m[2];
		else
			$basename = $pathParts['basename'];

		if ( $pathParts['dirname'] == ($this->main->plugin_url . $this->main->imagesDir) && $this->getExt($image) !== 'swf' )
			$thumb = $pathParts['dirname'] . '/t_' . $basename;
		else
			$thumb = $image;
	...
	function createTable( $tableName, $fields )
	{
		...
		if( $wpdb->get_var( "SHOW TABLES LIKE '".$tableName."'" ) != $tableName )
		{
			...
			$sql = "CREATE TABLE {$tableName} ({$columns}){$charset_collate}";
			$wpdb->query($sql);
		}
	}
		...
		foreach( $book->album->pages as $page )
		{
			if ($page->modified == 'true')
			{
				...
				if ( count($page->imgs) )
				{
					...
					$newPage = $this->buildImage( $images, $page, $book->zoomImageWidth, $book->zoomImageHeight, $page->template );

					if( $newPage )
					{
						$addPage = new pageflipPage( $this->main->imagesUrl . $newPage, $page->id, '' );
